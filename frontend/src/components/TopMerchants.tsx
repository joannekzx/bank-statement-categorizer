        ...
        <ol className="space-y-2">
          {data.map((m, i) => (
            <li key={m.merchant} className="flex items-center gap-3 text-sm">
              <span className="tnum w-5 text-right text-slate-400">{i + 1}</span>
              <span className="flex-1 truncate text-slate-700" title={m.merchant}>{m.merchant}</span>
              <span className="tnum text-slate-400">{m.count}×</span>
              <span className="tnum w-20 text-right font-medium text-slate-900">{formatMoney(m.total)}</span>
            </li>
          ))}
        </ol>
      )}
